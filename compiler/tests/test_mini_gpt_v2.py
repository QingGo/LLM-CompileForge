...
class MlirBuilder:
    ...
    def embedding(
        self, weight_name: str, weight_shape: tuple[int, ...], indices_var: str, indices_dims: tuple[int, ...]
    ) -> str:
        v = self._new_var("emb")
        ...
        self._lines.append(f'    {v} = "sf.embedding"({w_var}, {indices_var}) : ({wt}, {it}) -> {ot}')
        return v
    ...
    def layer_norm(self, input_var: str, input_dims: tuple[int, ...], w_name: str, b_name: str) -> str:
        v = self._new_var("ln")
    ...
    def sdpa(self, q_var: str, k_var: str, v_var: str, dims: tuple[int, ...], mask_var: str | None = None) -> str:
    ...
    def add(self, a_var: str, b_var: str, dims: tuple[int, ...]) -> str:
        v = self._new_var("add")
        it = self._tensor_type(*dims)
        self._lines.append(f'    {v} = "sf.add"({a_var}, {b_var}) : ({it}, {it}) -> {it}')
        return v

    def silu(self, input_var: str, dims: tuple[int, ...]) -> str:
        v = self._new_var("act")
        it = self._tensor_type(*dims)
        self._lines.append(f'    {v} = "sf.silu"({input_var}) : ({it}) -> {it}')
        return v

    def arange(self, start_val: int, size: int, dtype: str = "i64") -> str:
        v = self._new_var("arange")
        if dtype == "i64":
            self._lines.append(f"    %{v[1:]}_cst = arith.constant dense<{start_val}> : tensor<1xi64>")
            self._lines.append(f'    {v} = "sf.arange"(%{v[1:]}_cst) : (tensor<1xi64>) -> tensor<{size}xi64>')
        else:
            self._lines.append(f"    %{v[1:]}_cst = arith.constant dense<{float(start_val)}> : tensor<1xf32>")
    ...
    def gather(
        ...
        self._lines.append(f'    {v} = "sf.embedding"({w_var}, {indices_var}) : ({wt}, {idt}) -> {ot}')
        return v

    def build_module(
        self, func_name: str, func_args: list[str], func_body: list[str], ret_types: list[str], ret_vals: list[str]
    ) -> str:
        lines = ["module {"]
        lines.append(f"  func.func @{func_name}({', '.join(func_args)}) -> ({', '.join(ret_types)}) {{")
...
def _emit_linear(
    ...
    if has_bias:
        ...
        b.body_lines.append(
            f'    {v} = "sf.linear"({input_var}, {w_var}) : ({it}, tensor<{out_features}x{in_features}xf32>) -> {ot}'
        )
    return v
...
class NumpyReference:
    ...
    @staticmethod
    def softmax(x: np.ndarray, axis: int = -1, mask: np.ndarray | None = None) -> np.ndarray:
        if mask is not None:
    ...
    @staticmethod
    def sdpa(q: np.ndarray, k: np.ndarray, v: np.ndarray, scale: float, mask: np.ndarray | None = None) -> np.ndarray:
...
class MiniGpt:
    ...
    def build_mlir(self) -> str:
        ...
        return b.build_module("main_0", args, b.body_lines, ret_types, [final_ln])

    def compute_reference(self, input_ids: np.ndarray, weights: dict[str, np.ndarray]) -> np.ndarray:
        config = self._config
        ref = NumpyReference
...
class TestMiniGptV2:
    ...
    def _run_test(self, config: ModelConfig):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            actual = np.array((ctypes.c_float * int(np.prod(sz))).from_address(al), dtype=np.float32).reshape(sz)

            expected = model.compute_reference(input_ids, weights)
            cos = _cos(actual, expected)
            assert cos >= 0.9999, f"Mini GPT L={config.layers} mask={config.causal_mask}: cos={cos:.8f} < 0.9999"
