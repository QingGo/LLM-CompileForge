...
Tests that ``build_op_catalog()`` from ``compiler.dialect.op_catalog``
...
class TestBuildOpCatalog:
    ...
    def test_returns_non_empty_catalog(self) -> None:
        ...
        assert len(catalog.ops) >= 30, f"Expected at least 30 ops in catalog, got {len(catalog.ops)}"
    ...
    def test_all_ops_have_params(self) -> None:
        ...
        catalog = build_op_catalog()
        for op_def in catalog.ops:
            assert len(op_def.params) >= 1, f"Op '{op_def.name}' has no params"
    ...
    def test_catalog_contains_core_ops(self) -> None:
        ...
        expected = {"matmul", "add", "relu", "softmax", "layer_norm", "linear", "gelu", "silu", "view", "embedding"}
