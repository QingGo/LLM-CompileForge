...
def _make_mini_gpt_mlir(num_layers: int = 2) -> tuple[str, list[str], list[tuple[int, ...]]]:
    ...
    lines.append(f'    %h = "sf.embedding"(%tok_embed, %input_ids) : (tensor<{v}x{h}xf32>, tensor<{b}x{s}xi64>) -> tensor<{b}x{s}x{h}xf32>')

    # Position embedding — use sf.embedding for simplicity
    lines.append(f"    {w('pos_embed', (SEQ, HIDDEN))}")
    lines.append(f"    %pos_ids_0 = arith.constant dense<[0, 1, 2, 3]> : tensor<4xi64>")
    lines.append(f'    %pos = "sf.embedding"(%pos_embed, %pos_ids_0) : (tensor<{s}x{h}xf32>, tensor<4xi64>) -> tensor<4x{h}xf32>')
    lines.append(f"    %pos_bc = linalg.generic {{indexing_maps = [affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = [\"parallel\", \"parallel\", \"parallel\"]}} ins(%pos : tensor<4x{h}xf32>) outs(%h : tensor<{b}x{s}x{h}xf32>) {{")
    lines.append("    ^bb0(%pv: f32, %o: f32):")
    lines.append("      linalg.yield %pv : f32")
    lines.append(f"    }} -> tensor<{b}x{s}x{h}xf32>")
    lines.append(f'    %h1 = "sf.add"(%h, %pos_bc) : (tensor<{b}x{s}x{h}xf32>, tensor<{b}x{s}x{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')
    ...
    for layer in range(num_layers):
        ...
        lines.append(f'    %{p}_ln1 = "sf.layer_norm"({h_var}, %{p}_ln1_w, %{p}_ln1_b) : (tensor<{b}x{s}x{h}xf32>, tensor<{h}xf32>, tensor<{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')
        ...
        lines.append(f'    %{p}_q = "sf.linear"(%{p}_ln1, %{p}_q_w) : (tensor<{b}x{s}x{h}xf32>, tensor<{h}x{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')
        lines.append(f'    %{p}_k = "sf.linear"(%{p}_ln1, %{p}_k_w) : (tensor<{b}x{s}x{h}xf32>, tensor<{h}x{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')
        lines.append(f'    %{p}_v = "sf.linear"(%{p}_ln1, %{p}_v_w) : (tensor<{b}x{s}x{h}xf32>, tensor<{h}x{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')
        ...
        lines.append(f'    %{p}_attn = "sf.scaled_dot_product_attention"(%{p}_q, %{p}_k, %{p}_v) {{scale = {scale_val} : f64}} : (tensor<{b}x{s}x{h}xf32>, tensor<{b}x{s}x{h}xf32>, tensor<{b}x{s}x{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')

        # Output projection
        lines.append(f'    %{p}_attn_out = "sf.linear"(%{p}_attn, %{p}_o_w) : (tensor<{b}x{s}x{h}xf32>, tensor<{h}x{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')

        # Residual
        lines.append(f'    %{p}_res1 = "sf.add"({h_var}, %{p}_attn_out) : (tensor<{b}x{s}x{h}xf32>, tensor<{b}x{s}x{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')

        # Layer norm 2 (pre-FFN)
        lines.append(f"    {w(f'{p}_ln2_w', (HIDDEN,))}")
        lines.append(f"    {w(f'{p}_ln2_b', (HIDDEN,))}")
        lines.append(f'    %{p}_ln2 = "sf.layer_norm"(%{p}_res1, %{p}_ln2_w, %{p}_ln2_b) : (tensor<{b}x{s}x{h}xf32>, tensor<{h}xf32>, tensor<{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')
        ...
        lines.append(f'    %{p}_fc1 = "sf.linear"(%{p}_ln2, %{p}_fc1_w, %{p}_fc1_b) : (tensor<{b}x{s}x{h}xf32>, tensor<{fh}x{h}xf32>, tensor<{fh}xf32>) -> tensor<{b}x{s}x{fh}xf32>')
        lines.append(f'    %{p}_act = "sf.silu"(%{p}_fc1) : (tensor<{b}x{s}x{fh}xf32>) -> tensor<{b}x{s}x{fh}xf32>')
        lines.append(f'    %{p}_fc2 = "sf.linear"(%{p}_act, %{p}_fc2_w, %{p}_fc2_b) : (tensor<{b}x{s}x{fh}xf32>, tensor<{h}x{fh}xf32>, tensor<{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')

        # Residual
        lines.append(f'    %{p}_res2 = "sf.add"(%{p}_res1, %{p}_fc2) : (tensor<{b}x{s}x{h}xf32>, tensor<{b}x{s}x{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')
    ...
    lines.append(f'    %final_ln = "sf.layer_norm"({h_var}, %final_ln_w, %final_ln_b) : (tensor<{b}x{s}x{h}xf32>, tensor<{h}xf32>, tensor<{h}xf32>) -> tensor<{b}x{s}x{h}xf32>')
...
class TestMiniGpt:
    ...
    def test_mini_gpt_compiles_and_runs(self, num_layers):
        ...
        weights = {
            ...
            for n, s in zip(w_names, w_shapes)
        }
