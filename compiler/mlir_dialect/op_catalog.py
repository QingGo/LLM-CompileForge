...
def build_op_catalog() -> SfaOpCatalog:
    ...
    for kind in _OPS_BY_KIND:
        for name in _OPS_BY_KIND[kind]:
            ...
            op_def.name = name
            op_def.kind = kind
            for p in _OP_PARAMS.get(name, [_PIN2]):
...
def get_catalog_size() -> int:
    """Return the number of registered ops."""
    return sum(len(v) for v in _OPS_BY_KIND.values())


def get_ops_by_kind() -> dict[str, list[str]]:
    """Return a copy of the kind→op_names mapping."""
    return {k: list(v) for k, v in _OPS_BY_KIND.items()}
