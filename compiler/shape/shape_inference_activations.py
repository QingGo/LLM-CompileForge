...
_COMPARE_OPS: set[str] = {"gt", "lt", "eq", "ne", "le", "logical_and"}

_REDUCE_OPS: set[str] = {"mean", "sum", "var", "linalg_norm"}


# ── Assign infer_* functions from declarative sets ───────────

for _op in _BROADCAST_OPS:
    globals()[f"infer_{_op}"] = _infer_broadcast
for _op in _ELEMENTWISE_OPS:
    globals()[f"infer_{_op}"] = _infer_elementwise
for _op in _COMPARE_OPS:
    globals()[f"infer_{_op}"] = _infer_compare
for _op in _REDUCE_OPS:
    globals()[f"infer_{_op}"] = _infer_reduce
