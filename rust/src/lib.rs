...
//!
//! Exposes `PyScheduler` and `PyBlockManager` to Python via pyo3.
//! The bindings carry only metadata (integers, strings) across the
//! FFI boundary — Python is responsible for constructing PyTorch
//! tensors from the `Batch` metadata returned by `schedule()`.
...
impl PyBlockManager {
    ...
    fn allocate(&mut self, request_id: &str, num_tokens: usize) -> PyResult<Vec<usize>> {
        self.inner
    ...
    fn assign_cached_blocks(&mut self, request_id: &str, block_ids: Vec<usize>) {
        self.inner.assign_cached_blocks(request_id, &block_ids);
    }

    fn ensure_blocks(&mut self, request_id: &str, target_tokens: usize) -> PyResult<()> {
        self.inner
