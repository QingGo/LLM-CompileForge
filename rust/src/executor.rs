...
impl ModelExecutor {
    ...
    pub fn load_with_device(
        ...
        let executable: Box<dyn traits::Executable> = {
            ...
            let constants_path = dylib_dir.join("constants.bin");
            let hal_ir_path = dylib_dir.join("generated").join("hal_ir.json");
            // Use HalRustExecutable only when BOTH constants.bin AND
            // generated/hal_ir.json exist.  constants.bin alone is not
            // sufficient — it's present in all SFCF models, but hal_ir.json
            // is only generated by the HAL IR compiler pipeline.
            if constants_path.exists() && hal_ir_path.exists() {
                load_hal_rust_executable_from_dir(&dylib_dir)?
            } else {
