...
fn parse_safetensors_index(data: &[u8]) -> Result<HashMap<String, STensorInfo>, Box<dyn std::error::Error>> {
    ...
    if let Some(obj) = header.as_object() {
        for (key, info) in obj {
            ...
            index.insert(
                key.clone(),
                STensorInfo { dtype, shape, data_start: start, data_end: end },
            );
        }
    }
    Ok(index)
...
fn constant_to_f32(ct: &weight_loader::ConstantTensor) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
    match ct.dtype {
        tensor::Dtype::F32 => Ok(ct
            .data
            .chunks_exact(4)
            .map(|c| f32::from_le_bytes([c[0], c[1], c[2], c[3]]))
            .collect()),
        tensor::Dtype::F16 => Ok(ct
            .data
            .chunks_exact(2)
            .map(|c| half::f16::from_bits(u16::from_le_bytes([c[0], c[1]])).to_f32())
            .collect()),
        tensor::Dtype::BF16 => Ok(ct
            .data
            .chunks_exact(2)
            .map(|c| half::bf16::from_bits(u16::from_le_bytes([c[0], c[1]])).to_f32())
            .collect()),
        tensor::Dtype::I64 => Ok(ct
            .data
            .chunks_exact(8)
            .map(|c| i64::from_le_bytes([c[0], c[1], c[2], c[3], c[4], c[5], c[6], c[7]]) as f32)
            .collect()),
        tensor::Dtype::I32 => Ok(ct
            .data
            .chunks_exact(4)
            .map(|c| i32::from_le_bytes([c[0], c[1], c[2], c[3]]) as f32)
            .collect()),
        tensor::Dtype::I8 => Ok(ct.data.iter().map(|&b| b as i8 as f32).collect()),
        tensor::Dtype::U8 => Ok(ct.data.iter().map(|&b| b as f32).collect()),
    }
}
...
fn find_dylib(compiled_dir: &PathBuf) -> Result<PathBuf, Box<dyn std::error::Error>> {
    let entries = std::fs::read_dir(compiled_dir)
        .map_err(|e| format!("Cannot read compiled dir '{}': {}", compiled_dir.display(), e))?;
    for entry in entries.flatten() {
        if entry.path().extension().map(|ext| ext == "dylib").unwrap_or(false) {
...
fn find_safetensors(compiled_dir: &PathBuf) -> Result<String, Box<dyn std::error::Error>> {
    ...
    if meta_path.exists() {
        ...
        if let Some(source_path) = meta["weight_source"]["path"].as_str() {
            ...
            if p.extension().map(|e| e == "safetensors").unwrap_or(false) && p.exists() {
                return Ok(p.to_string_lossy().to_string());
            }
