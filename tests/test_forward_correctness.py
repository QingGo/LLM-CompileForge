...
def test_opt125m_compile_and_forward_cosine() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="l15_opt_") as tmpdir:
        mlir_mod = compile_mlir(
            model, example_args=(input_ids,), output_dir=tmpdir,
            apply_fusion=False,
        )
        print(f"  {len(mlir_mod.functions)} functions, "
              f"{sum(len(f.ops) for f in mlir_mod.functions)} ops")
        ...
        assert cos > 0.99, (
            f"Cosine similarity {cos:.6f} < 0.99"
        )
        assert py_np.shape == hf_logits.shape, (
            f"Shape mismatch: compiled {py_np.shape} vs HF {hf_logits.shape}"
        )
...
def test_opt125m_forward_smoke() -> None:
    ...
    config = OPTConfig.from_pretrained(
        os.path.join(snapshots, snap, "config.json")
    )
    config.use_cache = False
