...
def _load_hf_opt_125m():
    ...
    for snap in sorted(os.listdir(snapshots), reverse=True):
        ...
        if os.path.isfile(os.path.join(candidate, "pytorch_model.bin")):
            model_dir = candidate
            break
    assert model_dir is not None, f"No snapshot with pytorch_model.bin in {snapshots}"
...
class TestKVCachePrefill:
    ...
    def test_cache_policy_metadata_present(self):
        ...
        compiled_module = _load_compiled()
        assert "cache_policy" in compiled_module.metadata, "Compiled artifact missing cache_policy metadata"
    ...
    def test_forward_with_kv_api(self):
        ...
        assert logits.shape == (1, SEQ_LEN, 50272), f"Expected shape (1, {SEQ_LEN}, 50272), got {logits.shape}"
        assert isinstance(kv_tensors, list), f"Expected kv_tensors to be a list, got {type(kv_tensors)}"
    ...
    def test_function_split_with_cache_policy(self):
        ...
        assert len(a_funcs) == len(b_funcs), f"Mismatched a/b: {len(a_funcs)} a vs {len(b_funcs)} b"
        print(f"\n  Function split: {len(a_funcs)} a/b pairs, {len(compiled_module.functions)} total functions")
    ...
    def test_prefill_kv_extraction(self):
        ...
        assert hf_output.past_key_values is not None, "HF model with use_cache=True should return past_key_values"
        assert len(hf_output.past_key_values) == NUM_LAYERS, (
            f"Expected {NUM_LAYERS} layers in past_key_values, got {len(hf_output.past_key_values)}"
        )

        # HF past_key_values is DynamicCache (not subscriptable but iterable)
        pkv = list(hf_output.past_key_values)
        assert len(pkv) == NUM_LAYERS, f"Expected {NUM_LAYERS} layers in past_key_values, got {len(pkv)}"

        # Verify each layer's K/V shape (layers are (K, V, ...) tuples)
        for layer in range(NUM_LAYERS):
            hf_k = pkv[layer][0]
            hf_v = pkv[layer][1]
            assert hf_k.shape == (1, NUM_KV_HEADS, SEQ_LEN, HEAD_DIM), f"Layer {layer} K shape mismatch: {hf_k.shape}"
            assert hf_v.shape == (1, NUM_KV_HEADS, SEQ_LEN, HEAD_DIM), f"Layer {layer} V shape mismatch: {hf_v.shape}"

        print(f"\n  HF past_key_values: {NUM_LAYERS} layers, each K/V shape (1, {NUM_KV_HEADS}, {SEQ_LEN}, {HEAD_DIM})")
...
class TestKVCacheDecodeShape:
    ...
    def test_prefill_produces_correct_shape(self):
        ...
        assert prefill_logits.shape == (1, SEQ_LEN, 50272), f"Prefill shape: {prefill_logits.shape}"
...
class TestKVCacheMetadata:
    ...
    def test_hf_key_map_preserves_weight_mapping(self):
        ...
        assert "lm_head_weight" in hfk, "hf_key_map missing lm_head_weight"
