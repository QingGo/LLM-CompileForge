...
pub fn dump_layers(func_outputs: &[Vec<Tensor>]) {
    let Ok(dump_dir) = std::env::var("DUMP_LAYERS") else {
        return;
    };
    ...
    for (fi, outputs) in func_outputs.iter().enumerate() {
        for (oi, t) in outputs.iter().enumerate() {
            ...
            // Weight staging outputs can be 38M elements (lm_head); dumping
            // them on every step fills the disk without aiding layer triage.
...
fn write_npy(path: &str, data: &[f32], shape: &[usize]) -> std::io::Result<()> {
    ...
    let shape_str = shape
        ...
        .collect::<Vec<_>>()
        .join(", ");
    let mut header = if shape.is_empty() {
    ...
    // NPY v1: total header size (magic + version + u16 len + payload)
    // must be a multiple of 64.  The padding spaces are part of the
    // length stored in the u16 field.
