...
impl traits::Executable for HalRustExecutable {
    fn execute(
        ...
        let output_shapes: Vec<Vec<i64>> = outputs
            ...
            .map(|m| m.sizes_i64())
            .collect();
        let meta = Self::build_shape_meta_from_sfa(inputs, outputs);
        ...
        if op_name == "gather" && inputs.len() >= 2 {
            ...
            if inputs.len() >= 3 {
                ...
                let data_slice = unsafe { Self::sfa_as_f32_slice(data_sfa) };
                ...
                let weight_slice = unsafe { Self::sfa_as_f32_slice(weight_sfa) };
                // SAFETY: Indices buffer may be i64 or f32; accessed as raw bytes.
                let indices_bytes = unsafe { Self::sfa_as_bytes(indices_sfa) };
                // SAFETY: Output buffer pre-allocated by runner.
                let out_slice = unsafe { Self::sfa_as_f32_mut(out_sfa) };
        ...
        if op_name == "reshape" && inputs.len() >= 1 {
            ...
            let in_bytes = unsafe { Self::sfa_as_bytes(in_sfa) };
            // SAFETY: Output buffer accessed as mutable bytes for byte-level copy.
            let out_bytes = unsafe { Self::sfa_as_mut_bytes(out_sfa) };
        ...
        if op_name == "layer_norm" && inputs.len() >= 2 {
            ...
            let in_slice = unsafe { Self::sfa_as_f32_slice(in_sfa) };
        ...
        if op_name == "scaled_dot_product_attention" && inputs.len() >= 4 {
            ...
            let q_slice = unsafe { Self::sfa_as_f32_slice(q_sfa) };
            let k_slice = unsafe { Self::sfa_as_f32_slice(k_sfa) };
            let v_slice = unsafe { Self::sfa_as_f32_slice(v_sfa) };
            let mask_slice = unsafe { Self::sfa_as_f32_slice(mask_sfa) };
            // SAFETY: Output buffer pre-allocated by runner.
            let out_slice = unsafe { Self::sfa_as_f32_mut(out_sfa) };
        ...
        if op_name.starts_with("element_wise:") && inputs.len() >= 2 {
            ...
            if is_i64 {
                ...
                let a_bytes = unsafe { Self::sfa_as_bytes(a_sfa) };
                let b_bytes = unsafe { Self::sfa_as_bytes(b_sfa) };
            ...
            if is_f32 {
                let a_slice = unsafe { Self::sfa_as_f32_slice(a_sfa) };
                let b_slice = unsafe { Self::sfa_as_f32_slice(b_sfa) };
                let out_slice = unsafe { Self::sfa_as_f32_mut(out_sfa) };
        ...
        if op_name.starts_with("transpose") {
            ...
            let input_slice = unsafe { Self::sfa_as_f32_slice(in_sfa) };
