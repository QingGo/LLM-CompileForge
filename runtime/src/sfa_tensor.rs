...
impl SFATensor {
    ...
    /// Convert to a unified ``SfaMemRef`` HAL descriptor.
    ///
    /// Copies the raw descriptor and element-size metadata into an
    /// ``SfaMemRef``.  The data pointers reference the tensor's
    /// backing memory — the caller must ensure the tensor outlives
    /// the returned descriptor.
    pub fn as_sfa_memref(&self) -> crate::hal::sfa::SfaMemRef {
        use crate::hal::sfa::{SfaMemRef, SfaMemRefRaw};
        let raw = match &self.raw {
            SFATensorRawAny::R1(r) => SfaMemRefRaw::R1(*r),
            SFATensorRawAny::R2(r) => SfaMemRefRaw::R2(*r),
            SFATensorRawAny::R3(r) => SfaMemRefRaw::R3(*r),
            SFATensorRawAny::R4(r) => SfaMemRefRaw::R4(*r),
        };
        SfaMemRef {
            raw,
...
mod tests {
    ...
    // ── 3-way contract test (C / Rust / Python) ──────────────────────
    //
    // Prints sizeof and offsetof values in a parseable format consumed
    // by tests/contract/memref_3way.sh.  Run with:
    //   cargo test test_memref_3way_contract -- --nocapture

    #[test]
    fn test_memref_3way_contract() {
        println!("Rust:{}", std::mem::size_of::<SFATensorRaw1>());
