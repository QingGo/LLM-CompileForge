...
//! (Path B, pure-Rust).
//!
//! Only available when compiled with `--features hal-rust`.
...
fn find_safetensors(compiled_dir: &PathBuf) -> Option<String> {
    ...
    let hub_dir =
        PathBuf::from(&home).join(".cache/huggingface/hub/models--facebook--opt-125m");
...
/// Entry point for the forward_check_hal binary.
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "[forward_check_hal] HAL IR: {}",
        hal_ir_path.display()
    );
    println!(
        "[forward_check_hal] dylib (for weight access): {}",
        dylib_path.display()
    );
    println!(
        "[forward_check_hal] safetensors: {}",
        safetensors_path
    );
    ...
    println!(
        ...
        runner.hal_ir.num_functions, runner.hal_ir.model_name,
    );

    // Create HalRustExecutable and CPU stream.
    let hal_exe =
        crate::hal::rust::executable::HalRustExecutable::new(runner.hal_ir.num_functions);
    ...
    let input_ids: Vec<u32> = if let Ok(val) = std::env::var("FORWARD_CHECK_TOKENS") {
        let tokens: Vec<u32> = val
            ...
            .collect();
        eprintln!(
            "[forward_check_hal] Using tokens from FORWARD_CHECK_TOKENS={:?}",
            tokens
        );
        tokens
    } else {
    ...
    println!(
        "[forward_check_hal] Logits written to {}",
        csv_path
    );
    println!(
        "[forward_check_hal] Shape: {:?}, numel: {}",
        output.shape,
        logits.len()
    );
    println!(
        "[forward_check_hal] First 5 logits: {:?}",
        &logits[..5.min(logits.len())]
    );
    println!(
        "[forward_check_hal] Last 3 logits: {:?}",
